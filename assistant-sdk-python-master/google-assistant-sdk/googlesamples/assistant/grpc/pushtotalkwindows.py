...
speech_config = SpeechConfig(subscription="subscriptionkey", region="westeurope")
...
list_bewegen = ["cross", "raise"]
list_parts = ["arms", "hands", "hand", "arm"]


#Functie om ratios te berekenen voor taal
...
#De hoogste rate = de gedetecteerde taal
...
class SampleAssistant(object):
    ...
    def raise_hand(self, r):
        self.speak_message("english", "I will raise my arms for you")
        r.left_arm.shoulder_roll.goal_position = 0
        r.left_arm.arm_yaw.goal_position = 0
        r.left_arm.elbow_pitch.goal_position = 0
        r.left_arm.hand.forearm_yaw.goal_position = 0
        r.goto(
            goal_positions={'right_arm.shoulder_pitch': -140,
                            'right_arm.elbow_pitch': 0,
                            'right_arm.arm_yaw': -140,
                            'left_arm.shoulder_pitch': -140,
                            'left_arm.elbow_pitch': -0,
                            'left_arm.arm_yaw': -140,
                            },
            duration=2,
            wait=True,
        )
    ...
    def cross_arms(self, r):
        self.speak_message("english", "I will cross my arms for you")
        r.goto(
            goal_positions={
                'right_arm.shoulder_pitch': -90,
    ...
    #Het bericht wordt gesproken in de taal
    def speak_message(self, language, speaking_text):
        if (language == "dutch"):
            #Als de taal nederlands is, wordt de stem aangepast naar NL
            ssml = """<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="nl-NL-HannaRUS"><voice name="nl-NL-HannaRUS"><mstts:express-as style="cheerful">%s</mstts:express-as></voice></speak>""" % speaking_text

        else:
            #Anders standaard engels, er kunnen nog meerdere talen bheandled worden
            ssml = """<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="en-GB-HazelRUS"><voice name="en-GB-HazelRUS"><mstts:express-as style="cheerful">%s</mstts:express-as></voice></speak>""" % speaking_text

        synthesizer.speak_ssml_async(ssml).get()

    #Hoofdfunctie van de assistant
    def assist(self):
        global spokenAnswer
        global followUp
        global followUpSentence
        ...
        for resp in self.assistant.Assist(iter_log_assist_requests(),
            ...
            if len(resp.audio_out.audio_data) > 0:
                if not self.conversation_stream.playing:
                    ...
                    self.spokenQuestion = re.findall(r'"(.+?)"', str(self.spokenQuestion))
                        ...
                        if (followUp == True):
                            ...
                            for item in labeling["verbs"]:
                                ...
                                if(item["verb"] in list_bewegen):
                                    #Labelen via SRL
                                    subject = self.find_between(item["description"], "[ARG0: ", "]")
                                    subject2 = self.find_between(item["description"], "[ARGM-DIS: ", "]")
                                    verb = self.find_between(item["description"], "[V: ", "]")
                                    part = self.find_between(item["description"], "[ARG1: ", "]")

                                    #Are we talking to the robot?
                                    if(subject in matches or  subject2 in matches):
                                        if(verb == "cross"):
                                            # Als het gelabelde part in onze lijst zit
                                            if ([ele for ele in list_parts if (ele in part)]):
                                                self.cross_arms(robot)
                                        ...
                                        if (str(type[0]) == "ynQuestion" or str(type[0]) == "whQuestion"):
                                            speaking_text = "Do you want me to %s %s" % (verb, part)
    ...
    def storeAsText(self, text):
        self.spokenQuestion = text
